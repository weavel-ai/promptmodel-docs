...
If you use `FunctionModel`, `ChatModel` or `ChatAgent` in your code, you should add a `DevClient` object in that file so that they will be automatically detected and displayed in the dashboard.
...
extract_keyword_prompts = FunctionModel("extract_keyword").get_prompts()

# Or run FunctionModel directly
@client.register
def test():
    response = FunctionModel("choose_service").run_and_parse({})
    print(response.parsed_outputs)
    # Example ouptut : {"service_name" : "make_account", "arguments" : "[name, email, password]"}
```

The development dashboard will automatically detect your code changes and show `FunctionModel` instances in the dashboard.  
Now you can easily add FunctionModels in your IDE, and write prompts for them on our web editor without any additional work.
